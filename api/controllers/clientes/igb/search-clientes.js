...
module.exports = {
  ...
  friendlyName: 'Api Clientes Igb',

  description: 'Fuzzy search de los clientes para IGB desde archivo csv',
  ...
  fn: async function (inputs, exits) {
    ...
    let fileStream = fs.createReadStream(sails.config.custom.clientIgbCsv) // path.resolve(os.tmpdir(), 'fz3temp-3', 'product.txt')
    Papa.parse(fileStream, {
      ...
      error: err => {
        sails.log.error('Ocurrio un error - controllers/clientes/igb/search-clients', err)
        fileStream.destroy()
        exits.error(err)
      }
    })
